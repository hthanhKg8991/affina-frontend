...
import {
  getAllSuppliers,
  packagesGetAll,
  packagesGetBySupplier,
  postPackageBySupplier,
...
const BuyInsuranceGroupStep2Component = (props) => {
  ...
  const _renderTextViewAdditional = (item) => {
                  ...
                  <Stack className="justify-content-center">
                    ...
                    >
                      <input
                        ...
                        type="checkbox"
                        id={additionalItem._id}
                        checked={handleCheckAdditional(
                          additionalItem._id,
                          item.additional
                        )}
                        // checked={additionalItem.isChecked}
                        onChange={() =>
                          onSelectAdditional(additionalItem, item.package_code)
                        }
                      />
                      <label
                        htmlFor={additionalItem._id}
                        className="insure-package"
                      >
                        {additionalItem.name}
                      </label>
                    </Stack>
                    ...
                    {additionalItem.fee !== 0 ? (
                      <>
                        <p className="package-price">
                          {formatPrepaidAmount(
                            matchRound(additionalItem.amount)
                          )}
                          VNĐ
                        </p>
                        <p className="package-fee">
                          Phí:{" "}
                          {formatPrepaidAmount(matchRound(additionalItem.fee))}
                          /năm
                        </p>
                      </>
                    ) : (
    ...
    _templateListPackage = []
      ...
      .map((item, index) => {
        return (
          <Row
            className={
              item.package_code === isPackage.package_code ||
              item.package_code === step2.packageCode
                ? "group-item group-item-active cursor-pointer"
          ...
          >
              ...
              <div className="box-left text-center">
                <div className="wrap-image">
                  <Image
                    src={
                      item.supplier &&
                      configDefault.URL_IMG + item.supplier.image
                    }
                    srcSet={`${
                      item.supplier &&
                      configDefault.URL_IMG + item.supplier.image
            ...
            <Col md={9} xs={9} sm={9} className="box-right">
              ...
              >
                <Stack className="align-items-start">
                  ...
                  >
                    ...
                    {item.discount > 0 &&
                    !isStringNullOrEmpty(item.discount) ? (
                ...
                <div className="text-right ms-auto">
                  <p className="package-price">
                    {formatPrepaidAmount(item.price)}VNĐ
                  </p>
                  <p className="package-fee">
                    Phí: {formatPrepaidAmount(item.price_fee)}VNĐ/năm
                  </p>
                </div>
              </Stack>
              ...
              <Stack direction="horizontal">
                <div
                  className="procedure-text text-left"
                  onClick={() => handleSelectPackage(item, item.package_code)}
                >
              ...
              <Stack
                ...
                className="align-items-start"
              >
                {_renderTextViewAdditional(item)}
                <div className="text-right ms-auto">
                  {!isEmptyArray(packageRemain) && (
                    <p
                      className="preview-package"
                      onClick={(code, supplier) =>
                        handlePackageRemain(item.package_code, item.supplier)
                      }
                    >
  ...
  const _renderListPerson = () => {
    return listPerson.map((item, index) => {
      return (
        <Accordion flush defaultActiveKey="0" className="group-insurance">
          <Accordion.Item eventKey={index}>
            <Accordion.Header>
              <div className="text-header">
                <label className="text-uppercase">
                  {item.name} - <span>({genderByText(item.gender)})</span>
                </label>{" "}
                <br />
                <small className="">
                  Độ tuổi: {viewTextAge(item.birthday)}
                </small>
              </div>
            </Accordion.Header>
            <Accordion.Body>{_renderListPackage()}</Accordion.Body>
          </Accordion.Item>
        </Accordion>
      );
    });
            ...
            <Stack direction="horizontal">
              ...
              <Stack className="align-items-start">
                <Stack
                  ...
                  className="align-items-start"
                >
                  <label htmlFor={indexPackageMain} className="insure-package">
                    {packageRemainItem.name}
                  </label>
              ...
              <div className="text-right ms-auto">
                <p className="package-price">
                  {formatPrepaidAmount(packageRemainItem.price)}VNĐ
                </p>
                <p className="package-fee">
                  Phí: {formatPrepaidAmount(packageRemainItem.price_fee)}VNĐ/năm
                </p>
              </div>
            </Stack>
            <Line type="dashed" color="e6e6e6" />
    ...
    <Container className="insurance-groups-step2">
      <Container className="insurance-content-step2">
        <Row>
          ...
          <Col md={6}>
            <div className="insurance-search-center">
              <Row>
                <Col className="group-search">
                  <div className="input-group">
                    ...
                    <button
                      className="btn filter-vertical xs-visibility"
                      type="button"
                      onClick={handleViewFilterMobile}
                    >
              ...
              <div
                className="xs-visibility mobile-topic"
                onClick={handleViewBenefitMainMobile}
              >
        ...
        <Row>
          <Col md={3}>
            <div
              className={isFilterMobile ? "wrap-sidebar open" : "wrap-sidebar"}
            >
              <div className="wrap-mobile-filter background-gradient-to-bottom position-relative xs-visibility">
                <h5>Bộ lọc tìm kiếm</h5>
                <span className="close" onClick={handleViewFilterMobile}>
                  X
                </span>
              </div>
                ...
                <div className="insurance-sidebar">
                  ...
                  <div className="form-check">
                    <input
                      ...
                      name="radio"
                      id="selling"
                      onChange={() => handleCheckSort("selling")}
                    />
                  ...
                  <div className="form-check">
                    <input
                      ...
                      name="radio"
                      id="popular"
                      onChange={() => handleCheckSort("popular")}
                    />
                  ...
                  <div className="form-check">
                    <input
                      ...
                      name="radio"
                      id="discount"
                      onChange={() => handleCheckSort("discount")}
                    />
                      ...
                      return (
                        <div className="form-check" key={item._id}>
                          <input
                            ...
                            type="checkbox"
                            id={item.code}
                            onChange={() => handleSetSupplier(item._id)}
                          />
                          <label
            ...
            <div
              className={
                "insurance-sidebar bg-white sidebar-right-content my-sticky-top xs-hidden " +
                `${isBenefitMainMobile ? "open" : ""}` +
                ""
              }
            >
              <Form.Label className="justify-content-start">
                    ...
                    return (
                      <li key={itemMain._id}>
                        <div className="topic-benefit">
                          <span>{htmlParserCode(itemMain.description)}</span>
                        </div>
                        <b>
                          Số tiền được bảo hiểm: {numFormatter(itemMain.amount)}
                        </b>
                      </li>
                    );
                  })}
